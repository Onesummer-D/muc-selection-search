...
4. 白屏修复：`_register_frontend` 原用 `app.send_static_file()`（解析到 Flask 默认 static 目录），JS/CSS 全部 404 导致浏览器白屏；改为 `send_from_directory(dist, ...)` + SPA 回落，资源恢复 200。
5. 传统搜索先解析：首版传统模式把整句当关键词原文检索返回 0 条，与旧 Demo 行为不一致；改为传统模式先调 `/api/query/parse` 生成 QueryPlan 再检索（显式筛选优先），"放宽"基于上次检索参数删除对应字段重查。浏览器实测示例查询命中 1 条并显示带权重的匹配原因。
6. 详情表 grade 字段补中文标签"年级"。

## 浏览器实测（ZCode 内置浏览器，1280×720）

- 首页渲染正常：品牌、游客徽章、分段切换、示例按钮（截图存档）。
- 示例查询端到端：QueryPlan 面板显示 届别2026届/学历本科/专业计算机/城市西部/岗位基层；匹配原因带权重（+2/+3/+5/+4/+4）；命中 1 条，已核验徽章、来源、证据数可见。
- 详情弹窗：字段与证据并排表格，城市/专业带证据文本+bbox+ocr_rule，grade 显示"未提供"，游客提示"来源链接需校内登录后查看"，完整海报默认关闭提示。
